...
cd $GOPATH/src/nuage-mesos-cni
go build -o nuage-cni-mesos nuage-cni.go
go build -o nuage-cni-k8s nuage-cni.go
go build -o nuage-cni-openshift nuage-cni.go

cd /tmp
cp -r $GOPATH/src/nuage-mesos-cni nuage-cni-mesos-${version}
cp -r $GOPATH/src/nuage-mesos-cni nuage-cni-k8s-${version}
cp -r $GOPATH/src/nuage-mesos-cni nuage-cni-openshift-${version}
...
rpmbuild -ba $GOPATH/src/nuage-mesos-cni/rpmbuild/nuage-cni-mesos.spec
rpmbuild -ba $GOPATH/src/nuage-mesos-cni/rpmbuild/nuage-cni-k8s.spec
rpmbuild -ba $GOPATH/src/nuage-mesos-cni/rpmbuild/nuage-cni-openshift.spec
